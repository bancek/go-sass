...
// dart-source: lib/src/value/color/conversions.dart

// D50 is the D50 white point.
//
// Computed in init() to force IEEE 754 float64 division at runtime.
// Go constant expressions use arbitrary precision, which rounds
// 0.3457/0.3585 to a different float64 than Dart's IEEE 754 evaluation.
var D50 [3]float64

func init() {
	x := 0.3457
	y := 0.3585
	D50 = [3]float64{x / y, 1.0, (1.0 - x - y) / y}
}

// All matrices below follow https://www.w3.org/TR/css-color-4/#color-conversion-code.
// Direct pairs come from the spec; cross-space pairs were precomputed with
// https://gist.github.com/nex3/3d7ecfef467b22e02e7a666db1b8a316.

// lmsToOklab converts LMS cone responses to OKLab.
//
// Matches Dart: lmsToOklab in conversions.dart. It cannot be multiplied
// directly with xyzD65ToLms; XYZ to OKLab always routes through LMS with a
// cube-root step (see lmsSpace.convertInternal).
...
// oklabToLms converts OKLab back to LMS cone responses.
//
// Matches Dart: oklabToLms in conversions.dart. Like lmsToOklab it is only
// one leg of the XYZ<->OKLab path; the LMS values are cubed after this
// matrix (see oklabSpace.convertInternal).
...
var linearSrgbToLinearDisplayP3 = [9]float64{
...
var linearDisplayP3ToLinearSrgb = [9]float64{
...
var linearSrgbToLinearA98Rgb = [9]float64{
...
var linearRec2020ToLinearSrgb = [9]float64{
...
var linearSrgbToXyzD65 = [9]float64{
...
var xyzD65ToLinearSrgb = [9]float64{
...
var linearSrgbToLinearProphotoRgb = [9]float64{
...
var linearProphotoRgbToLinearSrgb = [9]float64{
...
var linearSrgbToXyzD50 = [9]float64{
...
var xyzD50ToLinearSrgb = [9]float64{
...
var linearDisplayP3ToLinearRec2020 = [9]float64{
...
var linearRec2020ToLinearDisplayP3 = [9]float64{
...
var linearDisplayP3ToXyzD65 = [9]float64{
...
var xyzD65ToLinearDisplayP3 = [9]float64{
...
var linearDisplayP3ToLms = [9]float64{
...
var lmsToLinearDisplayP3 = [9]float64{
...
var linearDisplayP3ToLinearProphotoRgb = [9]float64{
...
var linearProphotoRgbToLinearDisplayP3 = [9]float64{
...
var linearDisplayP3ToXyzD50 = [9]float64{
...
var xyzD50ToLinearDisplayP3 = [9]float64{
...
var linearA98RgbToLinearRec2020 = [9]float64{
...
var xyzD65ToLinearRec2020 = [9]float64{
...
var linearRec2020ToLms = [9]float64{
...
var lmsToLinearRec2020 = [9]float64{
...
var linearRec2020ToLinearProphotoRgb = [9]float64{
...
var linearProphotoRgbToLinearRec2020 = [9]float64{
...
var linearRec2020ToXyzD50 = [9]float64{
...
var xyzD50ToLinearRec2020 = [9]float64{
...
var xyzD65ToLinearProphotoRgb = [9]float64{
...
var linearProphotoRgbToXyzD65 = [9]float64{
...
var lmsToLinearProphotoRgb = [9]float64{
...
var linearProphotoRgbToLms = [9]float64{
...
var linearProphotoRgbToXyzD50 = [9]float64{
	0.79776664490064230, 0.13518129740053308, 0.03134773412839220,
	0.28807482881940130, 0.71183523424187300, 0.00008993693872564,
	0.00000000000000000, 0.00000000000000000, 0.82510460251046020,
}

var xyzD50ToLinearProphotoRgb = [9]float64{
	1.34578688164715830, -0.25557208737979464, -0.05110186497554526,
	-0.54463070512490190, 1.50824774284514680, 0.02052744743642139,
	0.00000000000000000, 0.00000000000000000, 1.21196754563894520,
}

// matrixMul applies a 3x3 matrix to a 3-element vector.
//
// FMA prevention: The Go compiler may fuse a*b+c into a single FMA instruction,
// which changes rounding behavior (single rounding vs two separate roundings).
// Dart does not use FMA, so results diverge across platforms (ARM64 vs AMD64 vs
// WASM) and between Go and Dart. Wrapping each multiply in float64() forces an
// intermediate rounding step, matching Dart's behavior.
//
// This pattern (float64(a*b) + float64(c*d)) must be applied to ALL a*b±c
// expressions in the color conversion pipeline. Use `go build
// -gcflags='-d=fmahash=/' ./value/...` to find compiler-generated FMA sites.
// See also: https://groups.google.com/g/golang-dev/c/Sti0bl2xUXQ
func matrixMul(m []float64, v0, v1, v2 float64) (float64, float64, float64) {
	if len(m) < 9 {
		return v0, v1, v2
	}
	return float64(m[0]*v0) + float64(m[1]*v1) + float64(m[2]*v2),
		float64(m[3]*v0) + float64(m[4]*v1) + float64(m[5]*v2),
		float64(m[6]*v0) + float64(m[7]*v1) + float64(m[8]*v2)
}

// convertColor bridges SassColor channel storage to a ColorSpace.Convert
// call, turning the missing-channel bitmask into nil pointers so absent
// channels propagate as missing rather than as zeros.
//
// Matches Dart: ColorSpace.convert invocation in SassColor.toSpace.
func convertColor(src, dst ColorSpace, c0, c1, c2 float64, alpha float64, missing [4]bool, extraMissing *bool) (*SassColor, error) {
	c0p := ptrFromMissing(c0, missing[0])
	c1p := ptrFromMissing(c1, missing[1])
	c2p := ptrFromMissing(c2, missing[2])
	ap := ptrFromMissing(alpha, missing[3])

	return src.Convert(dst, c0p, c1p, c2p, ap)
}

// ptrFromMissing returns nil for a missing channel and a pointer to v
// otherwise, matching Dart's nullable-double channel convention.
func ptrFromMissing(v float64, missing bool) *float64 {
	if missing {
		return nil
	}
	return &v
...
// ConvertLinearOpts carries optional parameters matching Dart's
// ColorSpace.convertLinear named parameters.
//
// Matches Dart: ColorSpace.convertLinear named params. Each flag records
// that the corresponding analogous channel set arrived missing, so the
// destination can mark its own analogous channels missing (#2810).
type ConvertLinearOpts struct {
	// MissingLightness reports that the source lightness channel is missing.
	MissingLightness bool
	// MissingChroma reports that the source chroma/saturation channel is missing.
	MissingChroma bool
	// MissingHue reports that the source hue channel is missing.
	MissingHue bool
	// MissingA reports that the source Lab/OKLab a channel is missing.
	MissingA bool
	// MissingB reports that the source Lab/OKLab b channel is missing.
	MissingB bool
}

// convertLinear is the default conversion pipeline using toLinear → matrix → fromLinear.
// Used by color spaces that don't override Convert.
//
// Matches Dart: ColorSpace.convertLinear. It first normalizes analogous
// missingness (a+b missing implies chroma+hue missing and vice versa), then
// routes polar destinations through their linear hub (HSL/HWB via sRGB,
// Lab/LCH via XYZ-D50, OKLab/OKLCH via LMS) before finishing the polar step
// in that hub's converter.
func convertLinear(src, dst ColorSpace, c0, c1, c2, alpha *float64, opts *ConvertLinearOpts) (*SassColor, error) {
	if dst == src {
		return newColorForSpaceInternalNoCheck(dst, c0, c1, c2, alpha), nil
	}
	...
	// color's channels are missing (#2810).
	...
	if (missingLightness && missingChroma && missingHue) || (c0 == nil && c1 == nil && c2 == nil) {
		return newColorForSpaceInternalNoCheck(dst, nil, nil, nil, alpha), nil
	}

	var linearDest ColorSpace
	switch dst {
	case HslColorSpace, HwbColorSpace:
		linearDest = SrgbColorSpace
	case LabColorSpace, LchColorSpace:
		linearDest = XyzD50ColorSpace
	case OklabColorSpace, OklchColorSpace:
		linearDest = LmsColorSpace
	default:
		linearDest = dst
	}

	c0v, c1v, c2v := 0.0, 0.0, 0.0
	if c0 != nil {
		c0v = *c0
	}
	if c1 != nil {
	...
	if linearDest == src {
		tr, tg, tb = c0v, c1v, c2v
	} else {
		linR := src.ToLinear(c0v)
		linG := src.ToLinear(c1v)
		linB := src.ToLinear(c2v)
		mat := src.TransformationMatrix(linearDest)
		r, g, b := matrixMul(mat, linR, linG, linB)
		tr = linearDest.FromLinear(r)
		tg = linearDest.FromLinear(g)
		tb = linearDest.FromLinear(b)
	}

	switch dst {
	case HslColorSpace, HwbColorSpace:
		return srgbSpace.convertInternal(dst, &tr, &tg, &tb, alpha, &SrgbConvertOpts{
			MissingLightness: missingLightness,
			MissingChroma:    missingChroma,
			MissingHue:       missingHue,
		})
	case LabColorSpace, LchColorSpace:
		c, err := xyzD50Space.convertInternal(dst, &tr, &tg, &tb, alpha, &XyzD50ConvertOpts{
			MissingLightness: missingLightness,
			MissingChroma:    missingChroma,
			MissingHue:       missingHue,
			MissingA:         missingA,
		...
		return c, nil
	case OklabColorSpace, OklchColorSpace:
		c, err := lmsSpace.convertInternal(dst, &tr, &tg, &tb, alpha, &LmsConvertOpts{
			MissingLightness: missingLightness,
			MissingChroma:    missingChroma,
			MissingHue:       missingHue,
			MissingA:         missingA,
	...
	default:
		...
		return newColorForSpaceInternalNoCheck(dst, rp, gp, bp, alpha), nil
	}
}

// labConvertComponentToF maps one XYZ component (scaled by the D50 white
// point) to Lab f-format, using the kappa/epsilon linear branch below the
// threshold and a cube root above it.
//
// Matches Dart: LabColorSpace internal f-mapping in xyz_d50.dart/lab.dart.
func labConvertComponentToF(component float64) float64 {
	if component > LabEpsilon {
		return sassmath.Pow(component, 1.0/3.0) + 0.0
	}
	return (float64(LabKappa*component) + 16) / 116 // prevent FMA: see matrixMul
}

// labFToXZ inverts labConvertComponentToF, mapping an f-format value back
// to an X or Z channel with the same epsilon-gated cube/linear split.
//
// Matches Dart: LabColorSpace internal inverse f-mapping.
func labFToXZ(component float64) float64 {
	cubed := component * component * component
	if cubed > LabEpsilon {
		return cubed
	}
	return (float64(116*component) - 16) / LabKappa // prevent FMA: see matrixMul
}
