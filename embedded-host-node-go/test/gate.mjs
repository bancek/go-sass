...
// LOCAL platform package to a real go-sass binary. Without this, stock
...
function platform() {
  // No musl detection: CI runs glibc, so the gnu-named package is what the
  // wrapper resolves here. On a musl host the vendored compiler-module.js
  // (upstream musl branch) resolves the linux-musl-* package instead.
  return `${process.platform}-${process.arch}`;
}

const triple = platform();
const platformPkg = `sass-embedded-go-${triple}`;
...
export const GO_SASS_BINARY = binary;
